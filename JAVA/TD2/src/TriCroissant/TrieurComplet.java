 ...
 */
public class TrieurComplet extends Thread {

  private int[] t; // tableau à trier
  private int debut, fin; // tranche de ce tableau qu'il faut trier
  private TrieurComplet parent;  // tread Trieur qui a lancé ce (this) Trieur
  private int nbNotify = 0; // Nombre de notifys envoyés à ce (this) Trieur

  public TrieurComplet(int[] t) {
    this(null, t, 0, t.length - 1);
  }
  
  private TrieurComplet(TrieurComplet parent, int[] t, int debut, int fin) {
    this.parent = parent;
    this.t = t;
    this.debut = debut;
    this.fin = fin;
  ...
  public void run() {
    if (fin - debut < 2) {
      ...
      TrieurComplet trieur1 = new TrieurComplet(this, t, debut, milieu);
      TrieurComplet trieur2 = new TrieurComplet(this, t, milieu + 1, fin);
  ...
  public static void main(String[] args) {
    ...
    TrieurComplet trieur = new TrieurComplet(t);
    try {
      trieur.join();
    }
    catch(InterruptedException e) {}
