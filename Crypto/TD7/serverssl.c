...
int main()
{
	...
	if(! SSL_CTX_use_certificate_file(ctx, "ssl/certs/monserveur.crt", SSL_FILETYPE_PEM))
	{
	...
	if (SSL_CTX_use_PrivateKey_file(ctx, "ssl/private/monserveur.key", SSL_FILETYPE_PEM) <= 0) {
	...
	if(SSL_CTX_load_verify_locations(ctx,"ssl/certs/ca.crt",NULL)<1){
